...
} from "./AdvancedTableContext"; // 가상 경로
import { AdvancedTableNormalRenderer } from "./renderers/AdvancedTableNormalRenderer"; // 가상 경로
import { AdvancedTableVirtualizedRenderer } from "./renderers/AdvancedTableVirtualizedRenderer"; // 가상 경로
import type {
  AdvancedTableProps,
  TableRenderingType,
} from "./types/tableTypes"; // 가상 경로
...
>(
  ...
  const { shouldUseInternalProvider = true } = props;

  if (!shouldUseInternalProvider) {
    // 외부 Provider를 사용하는 경우 (컨텍스트가 이미 상위에서 제공됨)
    return <TableRendererSelector {...props} />;
  }
  ...
  return (
    ...
    >
      ...
      headerRenderOptions={props.headerRenderOptions}
    >
      <TableRendererSelector
        shouldUseInternalProvider={shouldUseInternalProvider}
        {...props}
      />
    </AdvancedTableContextProvider>
  );
...
`TableRendererSelector` 컴포넌트에서 `props.renderingType` 값에 따라 `AdvancedTableVirtualizedRenderer` 또는 `AdvancedTableNormalRenderer` 컴포넌트를 선택적으로 렌더링하는 것을 볼 수 있습니다. 이 두 컴포넌트는 내부적으로 `BaseUITable`이라는 기본 UI 컴포넌트를 사용하여 테이블의 뼈대를 구성합니다.

**한 데이터 조회 페이지의 예시 코드**에서는 `renderingType="virtualized"`로 설정하여 가상화된 테이블을 사용하고 있습니다.

```typescript
import React from "react";
import AdvancedTable from "../components/table/AdvancedTable"; // AdvancedTable 컴포넌트 경로
import PageLayout from "../layout/PageLayout"; // 일반적인 페이지 레이아웃 컴포넌트로 가정
import { useFetchData } from "../api/dataService"; // 데이터 API 호출 훅 (일반화)
import { columnDefinitions } from "./configs/columnDefinitions"; // 컬럼 정의 (일반화)
import { LogEntry, LogFilter, LogSort } from "./types/logTypes"; // 예시 데이터 타입

/**
 * 데이터 조회 페이지
 * 다양한 시스템 데이터를 조회하는 페이지 컴포넌트
 */
const DataViewerPage: React.FC = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = useFetchData<LogEntry[]>("/api/logs"); // 예시 API 엔드포인트
  ...
  return (
    <PageLayout
      title="시스템 로그"
      description="시스템의 모든 활동 로그를 조회할 수 있습니다."
    >
      <div className="flex h-full w-full flex-1 flex-col">
        <AdvancedTable<LogEntry, LogFilter, LogSort>
          renderingType="virtualized" // 가상화 테이블 사용
          isFetching={isLoading}
          data={logData || []}
          initialData={logData || []} // 필터링/정렬 전의 원본 데이터로 사용 가능
          ...
          // bodyRowProps, options 등 기타 AdvancedTable props 설정
        />
      </div>
    </PageLayout>
  );
